...
class Submitter:
    def __init__(
        ...
        self.lsf_export_cmd = lsf_export_cmd
...
if __name__ == "__main__":
    ...
    ##################################
    # add additional lsf volumes
    ##################################
    lsf_volume_list = []
    
    lsf_volume_file = workdir / "lsf_volumes.txt"
    if lsf_volume_file.exists():
        with open( lsf_volume_file, 'r' ) as VOLUMES:
            for line in VOLUMES:
                line = line.strip()
                if len( line ) > 0:
                    lsf_volume_list.append( line )
    ##################################
    
    ##############################################################
    # build lsf docker volume to ensure we can see the directory #
    ##############################################################
    scratch_path = re.sub( u"^(/scratch1/fs1/[A-Za-z0-9]+)/.*$", u"\\1", str( workdir ) )
    
    lsf_dir_string = r'export LSF_DOCKER_VOLUMES="%s:%s' % ( scratch_path, scratch_path )
    
    if len( lsf_volume_list ) > 0:
        for vol_idx in range( len( lsf_volume_list ) ):
            current_volume = lsf_volume_list[ vol_idx ]
            lsf_dir_string = lsf_dir_string + " %s:%s" % ( current_volume, current_volume )
    
    lsf_dir_string = lsf_dir_string + '";'
    ##################################
